...
public partial class Hud : CanvasLayer
{
    ...
    private OutlinerView _outliner = null!;         // a coluna do estado à direita do mapa (o "outliner" do HoI4)
    ...
    public override void _Ready()
    {
        try
        {
            ...
            _outliner = new OutlinerView(); AddChild(_outliner); _outliner.Setup(_game);
            _outliner.OnOpen = OpenFromOutline;
            ...
            _map.OrderGiven += (from, to) => _multiSel.Order(from, to);   // arrastado do contador até à província
    ...
    /// <summary>Toque numa linha da coluna do estado: leva ao ecrã que trata daquilo. É metade do que a
    /// coluna do HoI4 vale — ver que a investigação acabou não serve de nada se depois é preciso procurar
    /// onde é que se escolhe a seguinte. O destino é da tabela (outline_section.target).</summary>
    private void OpenFromOutline(string target, int regionId)
    {
        if (_game.PlayerId is not int pid) return;
        switch (target)
        {
            case "foco": ClosePanels(); _focusTree.Open(pid); break;
            case "investigacao": ClosePanels(); _countryPanel.Open(pid, 4); break;   // aba Ciência
            case "producao": OpenProduction(); break;
            case "mapa": if (regionId > 0) ShowRegion(regionId); break;
            case "exercitos": OpenArmies(); break;
            case "ar": ClosePanels(); _warPanel.Open(1); break;                      // aba Ar
            case "mar": ClosePanels(); _warPanel.Open(2); break;                     // aba Mar
            case "decisoes": ClosePanels(); _countryPanel.Open(pid, 1); break;       // aba Decisões
        }
    }
    ...
    private void LayoutUnderTop()
    {
        ...
        _outliner?.PlaceUnder(y);           // a coluna do estado encosta-se logo por baixo da barra
    }
    ...
    private void SubscribeEvents()
    {
        ...
        // uma missão que acaba é notícia dos dois lados: cumprida paga poder político, falhada tira-o. Sem
        // este aviso o jogador só dava pela conta ao olhar para o cofre.
        _subs.Add(w.Events.Subscribe<DecisionMissionEnded>(e =>
        {
            if (_game.PlayerId == e.CountryId && _game.World.DecisionDefs.TryGetValue(e.DecisionId, out var md))
                Later(e.Met ? $"Missão cumprida: {md.Name} (+{md.RewardPolitical:0} de poder político)"
                            : $"Missão falhada: {md.Name} (−{md.FailPolitical:0} de poder político)",
                      e.Met ? Sfx.Kind.Fanfare : Sfx.Kind.Siren);
        }));
    ...
    private void RefreshAll()
    {
        try
        {
            ...
            _outliner.SetCovered(covered);
            if (_outliner.Visible) _outliner.Refresh();
    ...
    private string SmokeParties(int pid)
    {
        var w = _game.World;
        var c = w.Countries[pid];
        var rows = PartyView.Rows(w, c);
        if (rows.Count == 0) return "sem partidos na tabela";

        var top = rows[0];
        string who = w.PartyDefs.TryGetValue(c.Party, out var gov) ? gov.Name : "ninguém";

        // o empurrão da propaganda: paga-se poder político e a barra tem de subir mesmo
        var target = rows.FirstOrDefault(r => r.Def.Id != c.Party, rows[0]);
        float before = c.Political, had = c.Parties.GetValueOrDefault(target.Def.Id);
        c.Political = MathF.Max(c.Political, w.Rule("party_push_cost", 25f));
        _game.Dispatch(new PushPartyCommand(pid, target.Def.Id));
        float now = c.Parties.GetValueOrDefault(target.Def.Id);
        string push = now > had ? $"{target.Def.Name} de {had:0.0}% para {now:0.0}%" : $"{target.Def.Name} não mexeu";
        c.Political = before;

        // e o cartão desenhado, com uma chapa por partido (órfão: nunca entra na árvore de cena)
        var card = PartyView.Card(w, c, true, _ => { });
        int plates = card is null ? 0 : Glyph.Count(card).Drawn;
        card?.Free();

        float sum = c.Parties.Values.Sum();
        return $"{rows.Count} partidos ({string.Join(", ", rows.Select(r => $"{r.Def.Name} {r.Popularity:0}%"))}), "
             + $"soma {sum:0}%, governo d{(who.EndsWith("s") ? "os" : "o")} {who}, {PartyView.Clock(w, c)}, "
             + $"puxão do dia em {top.Def.Name} {PartySystem.Drift(w, c, top.Def):+0.000;-0.000;0}/dia, "
             + $"propaganda: {push}, {(PartyView.CoupWarning(w, c) is string cw && cw.Length > 0 ? cw : "rua calma")}, "
             + $"{plates} chapas no cartão";
    }

    /// <summary>O que o --smoke apanhou a passar a margem do telefone, painel a painel.</summary>
    private readonly List<Ui.TooWide> _fit = new();

    /// <summary>Mede um painel aberto contra a largura do telefone (Ui.Phone) e guarda quem não coube.
    /// Chama-se com o painel CHEIO e ainda aberto — depois de Close() já não há nada para medir.</summary>
    private void Fit(Control panel, string name) => Ui.Measure(panel, name);

    /// <summary>--smoke: o conteúdo cabe no ecrã? O jogo desenha numa tela de 1152 de largura no telefone
    /// mais estreito que suporta (um S24 Ultra ao alto), e o que pede mais do que isso sai pela direita —
    /// texto cortado a meio, chapas de "Activar" fora da vista. Isto mede cada painel que o smoke encheu e
    /// GRITA (erro) se alguma coisa não couber: a partir daqui um painel largo de mais não passa o portão.</summary>
    private string SmokeFits()
    {
        Fit(_top, "Barra de topo");
        Fit(_modeBar, "Modos de mapa");
        Fit(_outliner, "Coluna do estado");
        Ui.Watch = null;                                    // a régua desliga-se: fora do smoke ninguém mede nada
        if (_fit.Count == 0)
            return $"cabe tudo em {Ui.Phone:0} de largura (o mais largo é {Ui.Widest.Path} com {Ui.Widest.Wants:0}, "
                 + $"folga de {Ui.Phone - Ui.Widest.Wants:0})";

        var worst = _fit.OrderByDescending(b => b.Wants).Take(4)
                        .Select(b => $"{b.Path} {b.What} pede {b.Wants:0}");
        string report = $"{_fit.Count} controlos a passar a margem de {Ui.Phone:0}: {string.Join(" | ", worst)}";
        GD.PushError($"conteúdo fora do ecrã do telefone — {report}");
        return report;
    }

    /// <summary>--smoke: a ruptura. Diz quantos partidos sabem içar bandeira, quem no mundo de hoje está à
    /// beira do golpe e que guerras civis ardem, e a seguir o que o cartão do painel do País mostrou ao
    /// jogador — o aviso (quantas províncias se levantam e por que partido) ou a guerra já a arder.</summary>
    private string SmokeCivilWar(int pid)
    {
        var w = _game.World;
        var c = w.Countries[pid];
        string card = CivilWar.Of(w, c.Id) is (Country rebel, Country parent)
            ? $"cartão da guerra a arder: {rebel.Name} contra {parent.Name}"
            : CivilWar.Coming(w, c) is CivilWar.Forecast f
                ? $"cartão do aviso: {f.Regions} das {f.Held} províncias com {f.PartyName} ({f.Share:P0})"
                : "sem cartão a desenhar no país do jogador";
        return $"{CivilWar.Smoke(w)}; {card}";
    }

    /// <summary>--smoke: o quadro das decisões. Conta o que a tabela traz e quantas o país tem hoje ao
    /// alcance, desenha os cartões de TODAS as categorias (as fechadas incluídas — é lá que mora o motivo)
    /// e depois prova o ciclo a sério: assina uma, mede o multiplicador a mexer, e se for missão diz a meta
    /// e o dia do prazo. No fim devolve o país como estava: o smoke mostra, não joga.</summary>
    private string SmokeDecisions(int pid)
    {
        var w = _game.World;
        var c = w.Countries[pid];
        var cats = DecisionsView.Categories(w);
        if (cats.Count == 0) return "sem decisões na tabela";

        int cards = 0, plates = 0, shut = 0, ready = Decisions.Ready(w, c);
        foreach (var cat in cats)
        {
            var rows = Decisions.InCategory(w, c, cat.Id);
            cards += rows.Count;
            shut += rows.Count(r => r.Blocked is not null);
            var board = DecisionsView.Board(w, c, true, cat, _ => { });   // órfão: nunca entra na árvore de cena
            plates += Glyph.Count(board).Drawn;
            board.Free();
        }

        // o ciclo inteiro numa decisão, de preferência numa missão: enche-se o cofre, assina-se, mede-se, e
        // o país volta ao que era (a decisão sai da lista, a espera limpa-se e os multiplicadores refazem-se)
        float pp = c.Political, money = c.Money, men = c.Manpower, stab = c.Stability;
        c.Political = 999f; c.Money = 99999f; c.Manpower = 999999f; c.Stability = 100f;
        var pick = w.DecisionDefs.Values.OrderByDescending(d => d.IsMission).ThenBy(d => d.Sort).ThenBy(d => d.Id)
                                        .FirstOrDefault(d => Decisions.Check(w, c, d) is null);
        string ran;
        if (pick is null) ran = "nenhuma porta abre nem com o cofre cheio";
        else
        {
            _game.Dispatch(new ActivateDecisionCommand(pid, pick.Id));
            var act = Decisions.Active(w, pid, pick.Id);
            var eff = Decisions.Effects(w, pick.Id);
            string moved = eff.Count == 0 ? "sem multiplicador"
                : string.Join(" · ", eff.Select(e => $"{Decisions.StatName(w, e.Key)} ×{c.DecisionMult.GetValueOrDefault(e.Key, 1f):0.00}"));
            ran = act is null
                ? $"{pick.Name} não pegou"
                : $"assinada {pick.Name} ({Decisions.CostText(pick)}) → {moved}"
                + (pick.IsMission ? $", missão: {Decisions.GoalText(w, c, pick, act)}, prazo ao dia {act.MissionUntil}" : "");
            if (act is not null) w.ActiveDecisions.Remove(act);
            c.DecisionCooldownUntil.Remove(pick.Id);
            DecisionSystem.Recompute(w);
        }
        c.Political = pp; c.Money = money; c.Manpower = men; c.Stability = stab;

        return $"{DecisionsView.Smoke(w, pid, cards)} em {cats.Count} pastas ({shut} fechados com motivo, "
             + $"{ready} abertos), {plates} chapas desenhadas, {ran}";
    }
    ...
    /// <summary>--smoke: as campanhas diplomáticas. Prova o ciclo inteiro numa passagem — abre mesmo uma
    /// embaixada sobre um vizinho em paz, deixa correr uns dias de sistema, mede o que a opinião subiu, e
    /// depois fecha-a e vê a conta a desfazer-se. É a única forma de saber que o poder político sai, que o
    /// tecto trava e que a linha nova aparece na lista de razões do Relations.</summary>
    /// <summary>--smoke: a ordem arrastada. Pousa-se no contador de uma pilha nossa, puxa-se até um vizinho
    /// e larga-se; prova-se pela etiqueta que a seta mostrava e pelas divisões que ficaram com caminho.</summary>
    private string SmokeOrderDrag(int pid)
    {
        var w = _game.World;
        var from = w.Regions.Values.OrderBy(r => r.Id).FirstOrDefault(r => r.Neighbours.Count > 0
            && r.DivisionIds.Any(id => w.Divisions.TryGetValue(id, out var d) && d.CountryId == pid && d.Path.Count == 0));
        if (from is null) return "sem pilha nossa parada para arrastar";
        var mine = from.DivisionIds.Where(id => w.Divisions.TryGetValue(id, out var d) && d.CountryId == pid).ToList();
        foreach (int to in from.Neighbours.OrderBy(n => n))
        {
            if (!w.Regions.TryGetValue(to, out var dest)) continue;
            string seen = _map.SmokeDrag(from.Id, to);
            int marching = mine.Count(id => w.Divisions.TryGetValue(id, out var d) && d.Path.Count > 0);
            if (marching == 0) continue;                       // vizinho que a tropa não pode pisar: tenta o seguinte
            foreach (int id in mine) if (w.Divisions.TryGetValue(id, out var d)) d.ClearPath();
            return $"{from.Name} → {dest.Name}: a seta dizia «{seen}», {marching} divis{(marching == 1 ? "ão partiu" : "ões partiram")}"
                 + $"; seta viva depois de largar: {(_map.Order.Live ? "sim" : "não")}";
        }
        return $"{from.Name}: {from.Neighbours.Count} vizinhos e nenhum onde a tropa possa entrar";
    }

    /// <summary>--smoke: as pilhas do mapa. Prova que as caixas apanham o toque (e quantas delas assentam
    /// em terra do vizinho, que é o que tornava o toque no contador uma ordem errada), que a caixa marcada
    /// se desenha marcada, e que cada pilha diz o que está a fazer pela tabela stack_state.</summary>
    private string SmokeStacks(int pid)
    {
        var w = _game.World;
        var touch = _map.Regions.CounterTouch();
        var mine = w.Regions.Values.FirstOrDefault(r => r.DivisionIds.Any(id => w.Divisions.TryGetValue(id, out var d) && d.CountryId == pid));
        if (mine is not null) _map.Regions.PickCounters(new[] { mine.Id });
        var marks = _map.Regions.CounterMarks();
        _map.Regions.PickCounters(Array.Empty<int>());
        string ficha = mine is not null && Stacks.In(w, mine.Id, pid) is Stacks.StackRead s
            ? $"; {mine.Name}: {s.Count} divis{(s.Count == 1 ? "ão" : "ões")} {s.Name.ToLowerInvariant()} "
              + $"(org {s.Org:P0}, resistência {s.Hp:P0}, material {s.Kit:P0}, trincheira {s.Entrench:0.0})"
            : "";
        return $"{w.StackStates.Count} estados na tabela, {touch.Boxes} caixa{(touch.Boxes == 1 ? "" : "s")} "
             + $"({touch.Hit} apanham o toque, {touch.Stolen} assentam em terra do lado), "
             + $"{marks.WithState} com chapa de estado, {marks.Picked} marcada{(marks.Picked == 1 ? "" : "s")}; "
             + $"estados nossos: {Stacks.Report(w, pid)}{ficha}";
    }

    private string SmokeDiplo(int pid)
    {
        var w = _game.World;
        if (w.DiploActions.Count == 0) return "sem tabela de campanhas";
        var def = w.DiploActions.Values.OrderBy(a => a.Sort).ThenBy(a => a.Id).First();
        int target = 0;
        foreach (var o in w.Countries.Values.OrderBy(o => o.Id))
            if (o.Id != pid && !o.Capitulated && !w.AreAtWar(pid, o.Id) && w.SharesBorder(pid, o.Id)) { target = o.Id; break; }
        if (target == 0) return $"{w.DiploActions.Count} campanhas na tabela, sem vizinho em paz para provar";

        var me = w.Countries[pid];
        me.Political = MathF.Max(me.Political, def.CostStart + def.CostDay * 20f);
        float purse = me.Political, before = Relations.Opinion(w, target, pid);
        string? err = _game.Dispatch(new StartDiploDriveCommand(pid, target, def.Id));
        if (err is not null) return $"{w.DiploActions.Count} campanhas na tabela, recusada: {err}";

        var sys = new DiploDriveSystem();
        for (int i = 0; i < 10; i++) sys.Tick(w);
        var drive = DiploDriveSystem.Find(w, pid, target, def.Id);
        float peak = drive?.Progress ?? 0f, open = Relations.Opinion(w, target, pid);
        int reasons = Relations.Lines(w, target, pid).Count(l => l.Kind == "esforco");

        _game.Dispatch(new StopDiploDriveCommand(pid, target, def.Id));
        for (int i = 0; i < 3; i++) sys.Tick(w);
        float shut = drive?.Progress ?? 0f;
        int mundo = w.DiploDrives.Count;
        return $"{w.DiploActions.Count} na tabela ({string.Join(", ", w.DiploActions.Values.OrderBy(a => a.Sort).Select(a => a.Name))}), "
             + $"{def.Name} sobre {w.Countries[target].Name}: {peak:0.0} de {def.Cap:0} em 10 dias, "
             + $"opinião {before:+0;-0} → {open:+0;-0} ({reasons} linha nova), poder político {purse:0} → {me.Political:0}, "
             + $"fechada desfaz-se para {shut:0.0} em 3 dias; {mundo} campanha{(mundo == 1 ? "" : "s")} no mundo";
    }
    ...
    private void Smoke()
    {
        ...
        Ui.Watch = _fit;          // liga a régua do ecrã: cada aba de painel que o smoke encher mede-se sozinha
        ...
        _production.Open();
        _warPanel.Open(); _warPanel.SmokeDeal(); Fit(_warPanel, "Guerra"); _warPanel.Close();   // painel Guerra e mesa de negociação enchem sem rebentar
        _armyPanel.Open(); _armyPanel.Smoke(); Fit(_armyPanel, "Exércitos"); _armyPanel.Close();  // painel Exércitos: grupo criado, frente atribuída e dissolvido
        _end.Show(CampaignReport.Ongoing); Fit(_end, "Fim"); _end.Close();   // ecrã de fim de campanha, com o relatório todo
        ...
        // --smoke: o cartão da ruptura só aparece num país à beira do golpe, e ao dia 6 ninguém está lá.
        // Empurra-se o país do jogador para o precipício ANTES de encher as abas do País — assim o cartão
        // desenha-se e mede-se dentro do painel a sério — e devolve-se-lhe o governo logo a seguir: o smoke
        // mostra, não joga.
        var partiesWere = new Dictionary<string, float>(c.Parties);
        float stabWas = c.Stability;
        if (w.RebelStyles.Keys.OrderBy(p => p).FirstOrDefault(p => p != c.Party) is string rising)
        {
            foreach (var p in c.Parties.Keys.ToList()) c.Parties[p] = p == rising ? 65f : 5f;
        ...
        int chartDay = _countryPanel.SmokeChart(); Fit(_countryPanel, "País"); _countryPanel.Close();  // e o gráfico na nota de potência, com mira
        string civil = SmokeCivilWar(pid);                              // guerra civil: quem sabe levantar-se, quem está à beira, o que arde
        foreach (var (party, pop) in partiesWere) c.Parties[party] = pop;
        c.Stability = stabWas;
        ...
        var cronPlates = Glyph.Count(_journal); Fit(_journal, "Crónica"); _journal.Close();       // e as chapas dos géneros, na marca e nos filtros
        ...
        _region.Open(cap.Id); Fit(_region, "Região");
        ...
        // a fundura da árvore: quantos degraus este país pode fazer ao todo e quantos já tem. A árvore de
        // terra passou de três degraus por ramo a dez, e é isto que o diz sem se abrir o painel
        int techTotal = w.Techs.Values.Count(t => World.TechIsFor(t, c));
        int techDone = c.Techs.Count(t => w.Techs.ContainsKey(t));
        // quais ramos são do exército não se decide aqui: a coluna arm da tech_branch é que o diz
        int techArmy = w.Techs.Values.Count(t => World.TechIsFor(t, c)
                                             && w.TechBranches.TryGetValue(t.Branch, out var tb) && tb.Arm == "terra");
        ...
        int cmp = _compare.Smoke(rival); Fit(_compare, "Comparação"); _compare.Close();
        ...
        string carros = _production.SmokeTank();                         // prancheta dos carros: casco, ranhuras, peças e a marca assinada
        Fit(_production, "Produção");
        _production.Close();
        ...
        string fight = _battle.Smoke(cap.Id); Fit(_battle, "Batalha");                               // ecrã de batalha: os dois lados, linha e reserva
        int tree = _focusTree.Smoke(); Fit(_focusTree, "Focos");                                   // árvore de focos: grelha, traços e ramos rivais
        ...
        string schools = _doctrines.Smoke(); Fit(_doctrines, "Escolas");                              // escolas de guerra: ramos e degraus da árvore
        ...
        string pilhas = SmokeStacks(pid);                                 // e as pilhas: o contador é a unidade, e apanha o toque
        string arrasto = SmokeOrderDrag(pid);                             // e a ordem que se arrasta do contador até à província
        ...
        string partidos = SmokeParties(pid);                              // a opinião do país: barras dos partidos, urnas, propaganda e golpe
        string decisoes = SmokeDecisions(pid);                            // o quadro das decisões: pastas, portas, preços e as missões com prazo
        ...
        string estaleiro = _warPanel.SmokeYard();                         // estaleiro: casco, ranhuras, peças e a classe assinada
        ...
        string menu = _menu.Smoke(); Fit(_menu, "Menu");                        // menu de jogo: secções, botões e filas de chapas medidas
        int saves = _slots.Smoke(); Fit(_slots, "Jogos guardados");                          // e as fichas dos jogos guardados
