...
/// <summary>Grava amostras periódicas (dinheiro, divisões, regiões) do jogador e das maiores potências
/// para os gráficos de evolução. De history_sample_days em history_sample_days, no máximo history_tracked
/// países por amostra; persiste no save (s_history).</summary>
public sealed class HistorySystem : ISystem
{
    ...
    public void Tick(World w)
    {
        ...
        foreach (var c in picked)
            w.History.Add(new HistorySample(w.Clock.Day, c.Id, c.Money, divs.GetValueOrDefault(c.Id), regions.GetValueOrDefault(c.Id)));
    }
}
